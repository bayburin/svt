...
module Inventory
  class LkInventsController < ApplicationController
    ...
    def check_lk_authorization
      ...
      if @lk_auth.run
        user = User.find_by(tn: @lk_auth.lk_user_tn)
        sign_in :user, user

        user_iss = UserIss.find_by(tn: @lk_auth.data['user_id'])
        session[:id_tn] = user_iss.id_tn
      else
        render json: { full_messages: @lk_auth.data.errors.full_messages(', ') }, status: 403
      end
