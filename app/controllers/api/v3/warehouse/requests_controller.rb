      ...
      class RequestsController < ApplicationController
        ...
        def new_office_equipment
          ...
          request = ::Warehouse::Request.new(
            ...
            number_orbita: params['id'],

            number_lk: JSON.parse(params['parameters'])['common']['lk_id'],
        ...
        def answer_from_owner
          ...
          owner_id_tn = find_user("login==#{params[:sign_lk_user]}").first.try(:[], 'id')
        ...
        def request_files
          ...
          request.attachments.where(is_public: true).each do |attachment|
            array << {
              url: "https://#{ENV['APP_HOSTNAME']}.iss-reshetnev.ru/api/v3/warehouse/requests/download_attachment_request/#{attachment.id}",
