    ...
    this.Workplace.matchUploadFile(file).then(
      function (response) {
        ...
        if (!self.Item.matchDataFromUploadedFile(self.item, response.data.data)) {
          self.Flash.alert('Не удалось обработать данные. Убедитесь в том, что вы загружаете файл, созданный скачанной программой. Если ошибка не исчезает, обратитесь к администратору (т.50-32)');
