  ...
  module Requests
    ...
    class SendAnswerToUser < Warehouse::ApplicationService
      ...
      def send_file
        ...
        Orbita.add_event(@request.number_orbita, @current_user.id_tn, 'add_files', { is_public: true }, [file])
      end

      def send_answer
        payload = {
          message: 'Подтвердите, что вас устраивает расходный ордер на получение вычислительной техники',
          accept_endpoint: "https://#{ENV['APP_HOSTNAME']}.iss-reshetnev.ru/api/v3/warehouse/requests/answer_from_user/#{@request_id}"
        }

        Orbita.add_event(@request.number_orbita, @current_user.id_tn, 'to_user_accept', payload)
      end
    end
  end
end
