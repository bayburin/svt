  ...
  RSpec.describe WorkplaceCount, type: :model do
    ...
    describe 'create workpalce_count (one of the responsibles exists in the local table of users)' do
      ...
      context 'when users_attributes is not empty' do
        ...
        context 'when adds the same users several times' do
          let(:bayburin_user) { attributes_for(:bayburin_user) }
          subject { build(:active_workplace_count, users_attributes: [bayburin_user, bayburin_user]) }

          it { expect { subject.save! }.to raise_error(ActiveRecord::RecordInvalid) }
          it 'adds :multiple_user error to the :base key' do
            subject.valid?
            expect(subject.errors.details[:base].first).to include(error: :multiple_user, tn: bayburin_user[:tn])
          end
        end
        ...
        context 'when adds valid users' do
          context 'and when second user also already exists in local table' do
            ...
            subject do
              create(:active_workplace_count, users_attributes: [attributes_for(:bayburin_user), attributes_for(:user)])
            end
        ...
        context 'when adds invalid user' do
          ...
          subject do
            build(
              :active_workplace_count, users_attributes: [attributes_for(:bayburin_user), invalid_user]
            )
          end
    ...
    describe 'update workplace_count' do
      ...
      context 'when adds a few same users several times' do
        let(:role) { create(:lk_user_role) }
        # Удаляем лишние поля (чтобы было идентично данным, отправляемым клиентом)
        let(:bayburin_user) { attributes_for(:bayburin_user, role_id: role.id).except(:id_tn, :division, :email, :login, :fullname) }
        # Добавляем двух одинаковых пользователей
        let(:new_workplace_count) do
          2.times { loaded_workplace_count['users_attributes'] << bayburin_user }
          loaded_workplace_count
        end

        it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.to raise_error(ActiveRecord::RecordInvalid, /Ответственный для данного отдела с табельным 17664 уже существует \(либо вы его задали несколько раз\)/) }
      end
      ...
      context 'when adds valid user with custom phone' do
        context 'and when this user already exists in the local table of users' do
          let!(:bayburin_user) { create(:bayburin_user) }
          let(:new_user) do
            attributes_for(:bayburin_user, phone: '12-34').except(:id_tn, :division, :email, :login, :fullname)
          end
          let(:new_workplace_count) do
            loaded_workplace_count['users_attributes'] << new_user
            loaded_workplace_count
          ...
          it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.not_to change(User, :count) }
          it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.to change(WorkplaceResponsible, :count).by(1) }
        end

        context 'and when this user is not exist in the local table of users' do
          let(:role) { create(:lk_user_role) }
          let(:new_user) do
            attributes_for(:bayburin_user, role_id: role.id, phone: '12-34').except(:id_tn, :division, :email, :login, :fullname)
          end
          let(:new_workplace_count) do
            loaded_workplace_count['users_attributes'] << new_user
            loaded_workplace_count
          ...
          it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.to change(User, :count).by(1) }
          it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.to change(WorkplaceResponsible, :count).by(1) }
        end
      end
      ...
      context 'when adds invalid user' do
        ...
        it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.not_to change(User, :count) }
        it { expect { subject.update(new_workplace_count.deep_symbolize_keys) }.not_to change(WorkplaceResponsible, :count) }
        it 'adds :user_not_found error to the :base key' do
          subject.update(new_workplace_count.deep_symbolize_keys)
          expect(subject.errors.details[:base].first)
            .to include(error: :user_not_found, tn: invalid_user[:tn].to_s)
