  ...
  RSpec.describe Order, type: :model do
    ...
    describe '#set_consumer' do
      context 'when exists consumer_tn' do
        ...
        let(:new_user) { UserIss.find_by(fio: 'Кучеренко Виктор Николаевич') }
    ...
    describe '#compare_consumer_dept' do
      context 'when consumer_dept does not match with division of the selected item' do
        ...
        let(:item_to_orders) { [build(:item_to_order, inv_item: workplace_714.items.last)] }
        let(:operations) { [build(:order_operation, invent_item_id: workplace_714.items.last.item_id)] }
        subject { build(:order, item_to_orders: item_to_orders, operations: operations, consumer_dept: 715) }
      ...
      context 'when invent_item already does not have workplace_id (workplace was removed?)' do
        ...
        subject { build(:order, item_to_orders: item_to_orders, operations: [operation], consumer_dept: 715) }
