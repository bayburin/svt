...
module Warehouse
  RSpec.describe Order, type: :model do
    ...
    let(:employee) { build(:emp_kucherenko) }
    ...
    describe '#present_request_id' do
      ...
      context 'when status request is "create_order"' do
        ...
        it 'assign request_num for order' do
          ...
          expect(subject.request_num).to eq request.number_lk.to_s
        end
      end
    ...
    describe '#uniqueness_of_workplace' do
      let!(:workplace_1) { create(:workplace_pk, :add_items, items: %i[pc monitor], dept: 714) }
    ...
    describe '#set_consumer' do
      context 'when exists consumer_tn' do
        let(:tn) { 17_664 }
        let(:emp_user) { build(:emp_bayburin) }
        let(:emp_new_user) { build(:emp_kucherenko) }
        let(:result_subject) do
          subj = subject
          subj.consumer_fio = emp_user['fullName']
          subj.consumer_id_tn = emp_user['id']
      ...
      context 'when exists consumer_fio' do
        let(:fio) { 'Байбурин Равиль Фаильевич' }
        let(:tn) { 24_079 }
        let(:emp_new_user) { build(:emp_bayburin) }
        let(:emp_old_user) { build(:emp_kucherenko) }
    ...
    describe '#set_workplace_inv_items' do
      let!(:workplace) { create(:workplace_pk, :add_items, items: %i[pc monitor], dept: 714) }
      let(:operations) do
        [
    ...
    #     let!(:workplace_714) { create(:workplace_pk, :add_items, items: %i[pc monitor], dept: 714) }
    #     let!(:workplace_715) { create(:workplace_pk, :add_items, items: %i[pc monitor], dept: 715) }
    #     let(:operations) {
    #       [
    #         build(:order_operation, inv_items: [workplace_714.items.last]),
    #         build(:order_operation, inv_items: [workplace_715.items.last])
    #       ]
    #     }
    #     subject { build(:order, operations: operations, consumer_dept: 715) }
    ...
    #     let(:item) { create(:item, :with_property_values, type_name: :monitor) }
    #     let(:operation) { build(:order_operation, inv_items: [item]) }
    #     subject { build(:order, operations: [operation], consumer_dept: 715) }
    ...
    describe '#prevent_update_done_order' do
      ...
      context 'when status was done' do
        ...
        context 'and another attribute was changed' do
          let(:new_user) { create(:bayburin_user) }
