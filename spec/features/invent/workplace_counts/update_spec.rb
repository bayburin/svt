...
module Invent
  module WorkplaceCounts
    ...
    ' do
      ...
      given(:bayburin_user) { create(:bayburin_user) }
      given!(:workplace_count) { create(:active_workplace_count, users: [bayburin_user]) }

      background do
        sign_in user
        visit invent_workplace_counts_path

        expect(page).to have_content bayburin_user.fullname
      ...
      context 'when works with responsibles' do
        scenario 'tries to add valid responsible with custom phone', js: true do
          ...
          expect(page).to have_content 'Данные одела 714 обновлены'
          within '.table' do
        ...
        scenario 'tries to remove old and add a new responsible', js: true do
          ...
          within '.table' do
            expect(page).to have_content '714'
            expect(page).not_to have_content 'Байбурин Равиль Фаильевич'
            expect(page).to have_content '50-32'
            expect(page).to have_content 'Сильченко Дмитрий Михайлович'
          end
        end
        ...
        scenario 'tries to remove and add the same responsible', js: true do
          ...
          expect(page).to have_content 'Ответственный для данного отдела с табельным 17664 уже существует (либо вы его задали несколько раз)'
        end
