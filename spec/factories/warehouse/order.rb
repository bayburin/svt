module Warehouse
  FactoryBot.define do
    factory :order, class: Order do
      inv_workplace { nil }
      creator_id_tn { build(:emp_drag)['id'] }
      creator_fio { build(:emp_drag)['fullName'] }
      ...
      after(:build) do |order, ev|
        order.creator_fio = order.creator_id_tn ? build(:emp_drag)['fullName'] : ''
        order.validator_fio = order.validator_id_tn ? build(:emp_drag)['fullName'] : ''
