...
module Invent
  module WorkplaceCounts
    RSpec.describe Update, type: :model do
      ...
      let(:new_user) { build(:bayburin_user) }
      ...
      describe '#run' do
        ...
        context 'when adds new user' do
          context 'and when assigned a new role to user' do
            let(:role_id) { Role.find_by(name: 'lk_user').id }
            let(:user) { User.find_by(tn: new_user.tn) }

            it 'change the role as :lk_user' do
              subject.run
        ...
        context 'when adds present user' do
          ...
          let(:new_user) { create(:bayburin_user, role_id: role_id) }
