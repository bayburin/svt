...
module Invent
  module WorkplaceCounts
    RSpec.describe Create, type: :model do
      ...
      let(:user_new) { build(:bayburin_user) }
      ...
      context 'when adds new user' do
        let(:role_id_lk_user) { Role.find_by(name: 'lk_user').id }
        let(:user) { User.find_by(tn: user_new.tn) }

        it 'assigned a new role as :lk_user' do
          subject.run

          expect(user.role_id).to eq role_id_lk_user
        end
      end
