...
module Invent
  module Items
    RSpec.describe Index, type: :model do
      before do
        allow(UsersReference).to receive(:info_users).and_return([build(:emp_agureev)])
      end
      let(:user) { create(:user) }
      let(:workplace_count) { create(:active_workplace_count, users: [user]) }
      let!(:workplace) { create(:workplace_pk, :add_items, items: %i[pc monitor], workplace_count: workplace_count) }
      let!(:sec_workplace) { create(:workplace_pk, :add_items, items: %i[pc monitor], workplace_count: workplace_count, id_tn: 12_880) }
      ...
      context 'with filters' do
        ...
        context 'and with responsible filter' do
          let(:employee) { build(:emp_agureev) }
