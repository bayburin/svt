  ...
  module Orders
    RSpec.describe Swap, type: :model do
      before do
        ...
        allow_any_instance_of(Order).to receive(:find_employee_by_workplace).and_return([build(:emp_agureev)])
      end
      ...
      context 'when one of item from another division' do
        let!(:workplace_4) { create(:workplace_pk, :add_items, items: %i[pc monitor], dept: 714) }
