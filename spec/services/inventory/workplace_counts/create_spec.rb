...
module Inventory
  module WorkplaceCounts
    RSpec.describe Create, type: :model do
      ...
      let(:bayburin_user) { build :bayburin_user }
      let(:workplace_count) do
        tmp = build(:active_workplace_count, users: [test_user, bayburin_user]).as_json(include: :users)
      ...
      context 'with valid workplace_count params' do
        ...
        context 'when one user exists and second user not exists in database' do
          ...
          # end
          #
          # it 'adds the :lk_user role to created user' do
          #   subject.run
          #   expect(User.last.role).to eq Role.find_by(name: :lk_user)
          # end
        end
        ...
        context 'when users not exists yet in database' do
          ...
          it 'adds the :lk_user role to user' do
            subject.run
            expect(subject.data.users.all? { |user| user.role = Role.find_by(name: :lk_user) })
              .to be_truthy
          end
