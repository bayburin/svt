    ...
    module Invent
      RSpec.describe ItemsController, type: :controller do
        ...
        describe 'GET #search_items' do
          before do
            allow(UsersReference).to receive(:info_users).and_return([build(:emp_korjov)])
            allow_any_instance_of(User).to receive(:presence_user_in_users_reference).and_return([employee])
          end
          let(:employee) { build(:emp_korjov) }
          ...
          context 'when params is valid' do
            ...
            context 'and when data the parameters not present' do
              let(:params) { { barcode: 123, fio: 'fio', dept: 715, invent_num: 999_999 } }
